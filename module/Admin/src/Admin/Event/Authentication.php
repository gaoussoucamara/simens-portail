...
class Authentication extends AbstractActionController
{
    ...
    {
        //@todo - C'est ici qu'on utilise le plugin UserAuthentification pour verifier si l utilisateur est connecté par exemple
        ...
        if ($userAuth->hasIdentity()) {
            ...
            $pseudo = $userAuth->getIdentity();
            
            $user = $this->getUtilisateurTable()->getUtilisateursWithUsername($pseudo);

            //$role  = 'member'; //@todo - Get role from user!
            $role  = $user['role_personne'] ;
