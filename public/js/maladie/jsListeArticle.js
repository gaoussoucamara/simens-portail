    ...
    ( {
    					...
    					"oLanguage": {
    					   ...
    					   "sAjaxSource": ""+tabUrl[0]+"public/maladie/get-liste-article", 
...
//    }
    ...
    function suppression(id) {
    	$(null).w2overlay(null); alert('Attention vous etes sur le point de supprimer l\'article'); return false;
//    	$.ajax({
//            type: 'POST',
...
//        });
    ...
    function soumettre (id){
    	$('#soumettre-'+id).w2overlay({ html: "" +
			"" +
			"<div style='border-bottom:1px solid #49afcd; height: 30px; background: #f9f9f9; width: 200px; text-align:center; padding-top: 10px; font-size: 13px; color: #49afcd; font-weight: bold;'>Confirmer la soumission</div>" +
			"<div style='height: 50px; width: 200px; padding-top:10px; text-align:center;'>" +
			"<button class='btn' style='cursor:pointer;' onclick='popupFermer(); return false;'>Annuler</button>" +
			"<button class='btn' style='cursor:pointer;' onclick='soumission("+id+"); return false;'>Soumettre</button>" +
			"</div>" +
			"" 
    	});
    }
    ...
    function soumission(id) {
    	...
    	$.ajax({
            type: 'POST',
            url: tabUrl[0]+'public/maladie/soumission-article',
            data:'id='+id,
            success: function() {    
    ...
    //DESSOUMISSION DE L'ARTICLE
    //DESSOUMISSION DE L'ARTICLE
    function dessoumettre (id){
    	$('#dessoumettre-'+id).w2overlay({ html: "" +
			"" +
			"<div style='border-bottom:1px solid #49afcd; height: 30px; background: #f9f9f9; width: 210px; text-align:center; padding-top: 10px; font-size: 13px; color: #49afcd; font-weight: bold;'>Confirmer la dessoumission</div>" +
			"<div style='height: 50px; width: 210px; padding-top:10px; text-align:center;'>" +
			"<button class='btn' style='cursor:pointer;' onclick='popupFermer(); return false;'>Annuler</button>" +
			"<button class='btn' style='cursor:pointer;' onclick='dessoumission("+id+"); return false;'>Dessoumettre</button>" +
			"</div>" +
			"" 
    	});
    }
    
    function dessoumission(id) {
    	$(null).w2overlay(null);
    	$.ajax({
            type: 'POST',
            url: tabUrl[0]+'public/maladie/dessoumission-article',
            data:'id='+id,
            success: function() {    
            	vart=tabUrl[0]+'public/maladie/liste-article';
            	$(location).attr("href",vart);
